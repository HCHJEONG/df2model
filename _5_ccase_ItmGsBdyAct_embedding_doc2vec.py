# item gist act 임베딩 for 헌법판례 to (판결요지, embedding, 판례인용부호) 추천
...
if __name__ == '__main__':
    ...
    if embedding == True:
        ...
        df_ccase = pd.read_pickle(f'..//web2df//saved//{date_str}//openapi_detc.pickle')
        print(df_ccase.info())
        print(df_ccase[35: 40][['caseno', 'item', 'body', 'act', 'gist']])
        ...
        lines_ = [] # list[tuple(txt: string, cname: string)]
        with tqdm.tqdm(total = len(df_ccase), dynamic_ncols=True) as pbar:
            for idx, itm in df_ccase.iterrows():
                txt = ' '
                for fld in fields:
                    if type(itm[fld]) == type('string'):
                        txt = txt + itm[fld]
                lines_.append((txt, itm[cname]))
                pbar.update(1)
        lines_ = lines_[:s_no]
        print(f'totally {len(lines_)} lines')
        
        lines = []
        for x in tqdm.tqdm(lines_, dynamic_ncols=True):
            # if y == ' ':
            #     break
            if (type(x[0]) != type('string')) or (type(x[1]) != type('string')) or len(x[0]) < 2 or len(x[1]) <2:
                print('...')
                print(x[1])
                print('...')
                print(x[0])
        ...
        tasks = []
        for lines__ in lineslist:
            if(len(lines__)>0):
              tasks.append([lines__, engine])
        ...
        doc2vec_ccase = Doc2VecCCase(sentences) # [(사건명, [단어들])] -> TaggedDocument 객체
        doc2vec_model = Doc2Vec(doc2vec_ccase) # TaggedDocument 객체 (yield) -> doc2vec model 
