# items gists acts 임베딩 for 요지 to (판결요지, embedding, 판례인용부호와 판시사항 번호) 추천
...
class Doc2VecSummary:
    ...
    def __iter__(self):
        ...
        for idx, tt in enumerate(self.sentenceList):
            pbar.update(1)
            tag, text = tt # text는 한 sentence 단어들의 리스트
            # print(tag)
...
if __name__ == '__main__':
    ...
    if embedding == True:
        ...
        df_summary = pd.read_pickle(f'..//web2df//saved//{date_str}//df_glaw_corpus//df_glaw_summary_fullest.pickle')
        ...
        # 판결요지,사항, 법령 등을 전처리하여 저장함
        print("# doc2vec prepreproc")
        lines = [] # list[tuple(txt: string, cname: string)]
        with tqdm.tqdm(total = len(df_summary), dynamic_ncols=True) as pbar:
            for idx, itm in df_summary.iterrows():
                txt = ''
                for fld in fields:
                    if type(itm[fld]) == type('string'):
                        txt = txt + itm[fld]
                lines.append((txt, itm[cname].strip()+" "+"_^_"+str(itm[item_no])))
                pbar.update(1)
        lines = lines[:s_no]
        ...
        # prepreproc 구조:
        # list[list[tuple(token:string, leftPOS:string, cname: string)]] 하나의 요지에 대한 여러 단어 정보를 리스트로 만든 후 이 리스트들을 다시 하나의 리스트로 만든 것
        sentences = [] # list[tuple(cname: string, list[token: string])] 하나의 요지에 대한 식별 표지와 여러 단어들의 리스트를 튜플로 만든 후 이 튜플들을 다시 하나의 리스트로 만든 것
        ...
        # with open('./model/preproc_doc2vec.preproc', 'rb') as f:
        #     preproc =   pickle.load(f) # [(case full no , [tokens])]
        doc2vec_summary = Doc2VecSummary(sentences) # [(사건명, [단어들])] -> TaggedDocument 객체
        doc2vec_model = Doc2Vec(doc2vec_summary) # TaggedDocument 객체 (yield) -> doc2vec model 
