...
REMOTE_SERVER=lawvot@192.168.0.171
...
expect -c "
set timeout 2000
spawn scp -P 3330 ml_backend${IMAGE_TIMESTAMP}.tar ${REMOTE_SERVER}:/home/lawvot/docker_images/ml_backend_images
expect \"Enter passphrase for key\"
send \"${PW}\r\"
expect \"$\"
exit 0
...
sshpass -v -P phrase -p ${PW} ssh -p 3330 ${REMOTE_SERVER} 'bash -s' ${PW} ${ML_BACKEND_IMAGE} ${IMAGE_TIMESTAMP} < ./ml_backend_dev_deploy2.sh
